...
class CitiesController {
  ...
  show = async (req, res, next) => {
    const city = await CityModel.find(req.params.cityName);
    res.json(city);
  }
  ...
  _validateData = async (data, id) => {
    ...
    if (await this._checkIfCityExists(city.state_id, id)) {
      throw new Error(`The city at this province  "${city.state_id}" already registred.`);
    }

    return city;
  }

  _checkIfCityExists = async (state_id, id) => {
    const where = {
      state_id: state_id
    };
