...
class StatesController {
  ...
  show = async (req, res, next) => {
    const state = await StateModel.find(req.params.stateProvince);
    res.json(state);
  }
  ...
  _validateData = async (data, id) => {
    ...
    if (await this._checkIfEmailExists(state.province, id)) {
      throw new Error(`The state province "${state.province}" already cadastred.`);
    }

    return state;
  }

  _checkIfEmailExists = async (province, id) => {
    const where = {
      province: province
    };
