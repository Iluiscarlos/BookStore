const ENDPOINT = "http://177.44.248.50/newapi";
...
const findResults = () =>{
    ...
    const publishers = text.value.toLowerCase().replace(" " || "-", "%20");
    console.log(publishers+'%');
        axios.get(`${ENDPOINT}/publishers?name=${publishers}`)
        .then((response) => {
            if (response.status === 200) {
                const data = response.data;
                var trHTML = '';
...
const createCityCombo = async (id) =>{
    ...
    select += '<option value="0" selected>Select a Publishing</option>'
